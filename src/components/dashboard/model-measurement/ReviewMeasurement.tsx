...
import { MeasurementUpdate } from "./ModelMeasurementColumns";

export default function ReviewMeasurement({ id }: { id: string }) {
    const router = useRouter();
    const [update, setUpdate] = useState<MeasurementUpdate | null>(null);
    ...
    useEffect(() => {
        const fetchUpdate = async () => {
            try {
                const response = await fetch(`/api/model-measurement/${id}`);
    ...
    const handleAction = async (status_id: number, reason: string = "") => {
        setSubmitting(true);
        try {
            const response = await fetch("/api/model-measurement/approve-measurement", {
                method: "POST",
            ...
            const data = await response.json();
            if (response.ok && data.status === "success") {
                toast.success(`Measurement update ${status_id === 2 ? "Approved" : "Rejected"} successfully`);
                router.push("/admin/model-measurement");
            } else {
    ...
    return (
        <div className="mt-5 px-5">
            <Link href="/admin/model-measurement" className="flex items-center gap-2 text-whitefade hover:text-white mb-6">
                <ArrowLeft size={20} />
                Back to list
            </Link>

            <div className="max-w-4xl mx-auto bg-blackfade p-8 rounded-lg shadow-xl text-white">
                <h2 className="text-36 font-semibold mb-8">Review Measurement Update</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 border-b border-gray-700 pb-8">
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-medium text-default uppercase tracking-wider">
                                {update.measurement_type?.replace(/_/g, ' ')}
                            </p>
                        </div>
                    </div>
                ...
                <div className="mb-10">
                    <p className="text-gray-400 text-sm mb-4 uppercase font-bold tracking-widest">Measurement Proof Image</p>
            ...
            <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
                <DialogContent className="bg-blackfade2 text-white">
                    <DialogHeader>
                        <DialogTitle>Reject Measurement Update</DialogTitle>
                    </DialogHeader>
